...
{
  inputs_for_demog_ch <- c("intercept_coeff" = 1, "age_at_initial_pathologic_diagnosis" = 59, "ethnicityHISPANIC OR LATINO" = 0, "genderMALE" = 1, 
                           ...
                           "initial_pathologic_diagnosis_methodFine needle aspiration biopsy" = 0, "karnofsky_performance_score" = 20,
...
genetic_algorithm_for_optimal <- function(drug_vars, radiation_vars)
{
  ...
  GAmodel <- rbga.bin(size = n_options, popSize = 200, iters = 1, 
...
binary_programming_for_optimal <- function(vital.model)
{ 
  ...
  f.obj <- as.numeric(vital.model $coefficients)
...
run_show <- function()
{
  ...
  GAmodel <- genetic_algorithm_for_optimal(drug_vars, radiation_vars)
