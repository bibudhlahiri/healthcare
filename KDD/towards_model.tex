...
\KwIn{$D$: dataset on which we want to balance the positive and negative examples. Each element of $D$ is a tuple $(\mathbf{x},y)$ where $\mathbf{x}$ 
is the vector of the predictors and $y$ is the response variable.}
...
\tcp{$r$ and $q$ are the remainder and the quotient when $s$ is divided by $|D_P|$}\\
$q \gets s$ div $|D_P|$\\
$r \gets s \,\bmod\, |D_P|$\\
